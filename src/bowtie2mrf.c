...
static void printPairedData (BowtieEntry* prevEntry, BowtieEntry* currEntry, Stringa bufferSequence, Stringa bufferQuality, int sequenceLength) 
{
  ...
  if( strchr( prevEntry->chromosome, '|')  ) { // junction the first element
      ...
      numNucleotidesSecondExon = ((int)strlen(prevEntry->sequence)) - numNucleotidesFirstExon;
    ...
    stringPrintf( block1, "%s:%c:%d:%d:%d:%d",
		  ...
		  prevEntry->position + 1 + ((int)strlen(prevEntry->sequence)) - 1, // to be overly pedantic
		  1,
  ...
  if( strchr( currEntry->chromosome, '|')  ) { // junction the second element
      ...
      numNucleotidesSecondExon = ((int)strlen(currEntry->sequence)) - numNucleotidesFirstExon;
    ...
    stringPrintf( block2, "%s:%c:%d:%d:%d:%d",
		  ...
		  currEntry->strand,
		  currEntry->position + 1,
		  currEntry->position + 1 + ((int)strlen(currEntry->sequence)) - 1,
		  1,
		  ((int)strlen(currEntry->sequence)));
  }
